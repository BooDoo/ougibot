...
async function makeRandoDailyEmbed(hash) {
    ...
    let embed = new Discord.RichEmbed();
      ...
      **Accessibility:** ${meta.accessibility}`, true);

      embed.addField('Goal',
      `**Goal:** ${meta.goal}
      **Open Tower:** ${meta.tower}
      **Ganon Vulnerable:** ${meta.ganon}`, true);

      embed.addField('Gameplay',
      `**World State:** ${meta.mode}
      **Entrance Shuffle:** ${meta.entrance}
      **Boss Shuffle:** ${meta.boss}
      **Enemy Shuffle:** ${meta.enemy}
      **Hints:** ${meta.hints}`, true);

      embed.addField('Difficulty',
      `**Swords:** ${meta.weapons}
      **Item Pool:** ${meta.pool}
      **Item Functionality:** ${meta.functionality}
      **Enemy Damage:** ${meta.damage}
      **Enemy Health:** ${meta.health}`, true);
